...
def decode_predictions(scores, bboxes, threshold):
    ...
    if scores.shape[1] > 1:
        # If scores have more than one class, extract the face class (assumed to be index 1)
        face_scores = scores[:, 1]
    else:
        # If scores have shape (N, 1), squeeze to (N,)
...
def adjust_scale(bboxes, scale_x, scale_y):
    ...
    bboxes[:, [0, 2]] *= scale_x *5  # Scale x-coordinates
    bboxes[:, [1, 3]] *= scale_y *5 # Scale y-coordinates
    return bboxes
...
while True:
    ...
    final_bboxes, final_scores = post_process(
        outputs_dict, img_shape=(640, 640), conf_threshold=0.5, nms_threshold=0.4
    )
