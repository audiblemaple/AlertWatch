...
class Transforms():
    ...
    def crop_face(self, image, landmarks, crops):
        ...
        landmarks = torch.tensor(landmarks).clone().detach()
    ...
    def random_eye_crop(self, image, landmarks):
        ...
        img_shape = np.array(image).shape
        landmarks = torch.tensor(landmarks).clone().detach()
        landmarks = landmarks - torch.tensor([[min_x, min_y]])
        landmarks = landmarks / torch.tensor([img_shape[1], img_shape[0]])
        return TF.resize(image, (224, 224)), landmarks  # Ensure the size is consistent
...
if __name__ == "__main__":
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        model_save_path = f'weights_2000_epochs/face_landmarks_epoch_{epoch}.pth'
