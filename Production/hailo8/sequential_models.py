...
def preprocess_face(frame, bbox, input_shape, gray=True):
    ...
    preprocessed_face = np.ascontiguousarray(preprocessed_face)
    ...
    return preprocessed_face, bbox, transformation_params
...
async def main():
    ...
    print(f"face_det_input_name: {face_det_input_name}")
    print(f"face_det_output_name: {face_det_output_name}")
    ...
    while True:
        ...
        outputs = {
            'scrfd_10g/conv54': raw_faces[output_names['bbox_pred']],
            'scrfd_10g/conv53': raw_faces[output_names['cls_pred']]
        }

        faces = postprocess_faces(outputs, img_w, img_h, scale, pad_w, pad_h)


        # print("Model 1 Output:", faces)

        # Example usage
        # Replace `output` with actual model output
        ...
        for (x1, y1, x2, y2, score) in faces:
            ...
            preprocessed_face, adjusted_bbox, transformation_params  = preprocess_face(frame, (x1, y1, x2, y2), face_landmarks_input_shape)
            ...
            try:
                ...
                # landmarks_batch = landmarks_batch / (transformation_params['scale'])
        ...
        for landmarks in all_landmarks:
            for (x, y) in landmarks:
                cv2.circle(original_frame, (int(x), int(y)), 1, (0, 255, 0), -1)
