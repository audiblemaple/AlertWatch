...
async def main():
    ...
    input_shape_1 = hailo_inference.get_input_shape(model_id=1)
    input_shape_2 = hailo_inference.get_input_shape(model_id=2)
    print(f"Model 1 Input Shape: {input_shape_1}")
    print(f"Model 2 Input Shape: {input_shape_2}")
    ...
    while True:
        ...
        processed_frame, scale, pad_w, pad_h, img_w, img_h = preprocess_faces(
            frame, input_size=(input_shape_1[0], input_shape_1[1])
        )
        processed_frame = processed_frame.astype(np.float32)
        processed_frame = np.expand_dims(processed_frame, axis=0)

        # Run inference on Model 1 (Face Detection)
        raw_faces = hailo_inference.run(model_id=1, input_data=processed_frame)
        faces = face_detection(raw_faces, frame, input_shape_1[0], input_shape_1[1])
        ...
        for (x1, y1, x2, y2, score) in faces:
            ...
            # preprocessed_face, adjusted_bbox = process_face(frame, (x1, y1, x2-x1, y2 -y1), input_shape_2)
            preprocessed_face, adjusted_bbox = process_face(frame, (x1, y1, x2-x1, y2 -y1), input_shape_2)

            if preprocessed_face is None:
                continue

            # Run inference on Model 2 (Landmarks Detection)
